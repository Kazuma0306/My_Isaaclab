...
def stepping_stones_terrain(difficulty: float, cfg: hf_terrains_cfg.HfSteppingStonesTerrainCfg) -> np.ndarray:
    ...
    length_pixels = int(cfg.size[1] / cfg.horizontal_scale)
    # -- stones
    stone_distance = max(1, round(stone_distance / cfg.horizontal_scale))
    ...
    if length_pixels >= width_pixels:
        while start_y < length_pixels:
            ...
            # ズレの最大幅を、石の幅の20%までに制限する
            # max_shift = int(stone_width * 0.2)
            # # max_shiftが1以上の場合のみランダムなズレを適用し、0の場合はズレをなくす
            # if max_shift > 0:
            #     start_x = np.random.randint(0, max_shift)
            # else:
            #     start_x = 0
        ...
        # # max_shiftが1以上の場合のみランダムなズレを適用し、0の場合はズレをなくす
        # if max_shift > 0:
        #     start_y = np.random.randint(0, max_shift)
        # else:
        #     start_y = 0
